...
class Paypal_lib
{
	...
	function __construct()
	{
		...
		$sanbox = $this->CI->general_settings['paypal_sandbox'];
		$sandbox_url = $this->CI->general_settings['paypal_sandbox_url'];
		$live_url    = $this->CI->general_settings['paypal_live_url'];
		$business_email = $this->CI->general_settings['paypal_email'];
		$currency_code = $this->CI->general_settings['currency'];

		$this->paypal_url = ($sanbox == 1) ? $sandbox_url : $live_url;

		$this->last_error = '';
		$this->ipn_response = '';

		$this->ipn_log_file = TRUE;
		$this->ipn_log = BASEPATH . 'logs/paypal_ipn.log';

		$this->button_path = $this->CI->config->item('paypal_lib_button_path');

		// populate $fields array with a few default values.
		$this->add_field('business', $business_email);
		$this->add_field('rm', '2');
	...
	{
		$str = '';
		$str .= form_open($this->paypal_url, 'name=' . $form_name) . "\n";
