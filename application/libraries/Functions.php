	...
	function file_insert($location, $controlname, $type, $size)
	{
		...
		if (isset($_FILES[$controlname]) && $_FILES[$controlname]['name'] != NULL) {
			...
			if ($type == 'image') {
				...
				if ($file_extension == 'pdf' || $file_extension == 'jpg' || $file_extension == 'jpeg' || $file_extension == 'png' || $file_extension == 'gif' || $file_extension == 'doc' || $file_extension == 'docx' || $file_extension == 'txt' || $file_extension == 'rtf') {
					if ($filesize <= $size) {
						$return['msg'] = $this->file_upload($location, $controlname);
						$return['status'] = 1;
					} else {
					...
					$return['msg'] = 'File Must Be In pdf,jpg,jpeg,png,gif,doc,docx,txt,rtf Format';
					$return['status'] = 0;
				}
			} elseif ($type == 'excel') {
