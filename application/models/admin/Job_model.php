...
class Job_Model extends CI_Model
{
	...
	public function get_job_by_id($job_id,$admin_id){
		$query = $this->db->get_where('xx_job_post', array('id' => $job_id ));
	...
	{
		$this->db->select('xx_employers.id as emp_id, 
			xx_employers.firstname, 
			xx_employers.lastname,
			xx_companies.id as company_id, 
		...
		$this->db->order_by("xx_employers.created_date", "DESC");
		$query = $this->db->get();
		$module = array();
		if ($query->num_rows() > 0) 
		{
